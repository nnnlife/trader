...
def check_one_third(candle, trendpoint):
    ...
    for onemin in candle['raw']:
        ...
        if long_trader is not None:
            if onemin['highest_price'] > long_trader['max_index']:
                long_trader['max_index'] = onemin['highest_price']

            diff = (long_trader['max_index'] - long_trader['index']) / long_trader['index'] * 100.
            if diff > 1.0:
                one_third = long_trader['max_index'] - (long_trader['max_index'] - long_trader['index']) * 0.5
                if onemin['highest_price'] != long_trader['max_index'] and onemin['lowest_price'] <= one_third:
                    long_price = get_etf_price(today_leverage_data, dt)
                    #print('cut long', 'buy index', long_trader['index'], 'one_third', one_third)
                    close_position(long_trader, long_price, dt)
                    long_trader = None
...
if __name__ == '__main__':
    ...
    while start_date < datetime(2019, 6, 1).date():
