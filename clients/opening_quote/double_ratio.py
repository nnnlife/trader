    ...
    for progress, code in enumerate(codes):
        ...
        for i, t in enumerate(ticks):
            if not is_started:
                ...
                if nine_ten is None and t['date'] >= datetime.combine(t['date'].date(), time(9, 0, 16)):
                    nine_ten = (bias_amount > 0, all_amount / quote_amount, t['current_price'] > open_price)
...
if __name__ == '__main__':
    ...
    while start_dt <= datetime(2020, 8, 10).date():
        ...
        sorted_by_ratio = list(filter(lambda x: x['nine_ten_cum_plus'] and x['nine_ten_high'], sorted_by_ratio))
        sorted_by_ratio = sorted(sorted_by_ratio, key=lambda x: x['nine_ten_ratio'], reverse=True)

        filtered_codes = [t['code'] for t in sorted_by_ratio[:5]]
        for sbr in sorted_by_ratio[:5]:
